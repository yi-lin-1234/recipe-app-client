...
import { getAllRecipes, groupAndCountRecipesByCuisine } from "../apiService";
import GridList from "../components/GridList";
import Error from "./Error";
import { Recipe, GroupedDataCuisineObj } from "../type";
...
function GroupByCuisine() {
  ...
  const [groupedData, setGroupedData] = useState<GroupedDataCuisineObj[]>([]);
  ...
  const navigation = groupedData.map((item) => ({
    name: item.cuisine,
    count: item.count,
    current: currentTab === item.cuisine,
  }));

  const filteredRecipes = recipes.filter(
    (recipe) => recipe.cuisine === currentTab
  );
  ...
  useEffect(() => {
    async function initialSetUp() {
      ...
      try {
        ...
        setRecipes(data);
        const groupDate = await groupAndCountRecipesByCuisine();
